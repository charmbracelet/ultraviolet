...
func TestStyledString(t *testing.T) {
	...
	for i, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if ss.Buffer.Width() != tc.expectedWidth {
				t.Errorf("case %d expected width %d, got %d", i+1, tc.expectedWidth, ss.Buffer.Width())
			}
			if ss.Buffer.Height() != tc.expectedHeight {
				t.Errorf("case %d expected height %d, got %d", i+1, tc.expectedHeight, ss.Buffer.Height())
			}
